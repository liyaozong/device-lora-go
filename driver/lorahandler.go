...
// Copyright (c) 2021-2023 Starblaze Ltd
//
...
func (handler LoraHandler) addContext(next echo.HandlerFunc) echo.HandlerFunc {
...
func (handler LoraHandler) processAsyncRequest(c echo.Context) error {
...
func (handler LoraHandler) readBody(request *http.Request) ([]byte, error) {
...
func deviceHandler(c echo.Context) error {
	handler, ok := c.Request().Context().Value(handlerContextKey).(LoraHandler)
	if !ok {
		return c.String(http.StatusBadRequest, "Bad context pass to handler")
	}
	handler.logger.Infof("deviceHandler")
...
func validateCommandValue(resource model.DeviceResource, reading interface{}, valueType string, contentType string) (interface{}, error) {
	...
	case common.ValueTypeUint8:
		...
		if err := checkUintValueRange1(valueType, val); err != nil {
			return nil, err
		}
	...
	case common.ValueTypeInt8:
		...
		if err := checkIntValueRange1(valueType, val); err != nil {
			return nil, err
		}
	...
	case common.ValueTypeFloat32:
		...
		if err := checkFloatValueRange1(valueType, val); err != nil {
			return nil, err
		}
...
func checkUintValueRange1(valueType string, val interface{}) error {
	switch valueType {
	case common.ValueTypeUint8:
		_, ok := val.(uint8)
		if ok {
...
func checkIntValueRange1(valueType string, val interface{}) error {
	switch valueType {
	case common.ValueTypeInt8:
		_, ok := val.(int8)
		if ok {
...
func checkFloatValueRange1(valueType string, val interface{}) error {
	switch valueType {
	case common.ValueTypeFloat32:
		_, ok := val.(float32)
		if ok {
